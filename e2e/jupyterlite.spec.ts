/**
 * JupyterLite smoke test (project `jupyterlite`, served by
 * scripts/serve-lite.mjs on 8000 with COOP/COEP — see playwright.config.ts).
 *
 * The notebook is executed the way a user would — Run menu → Run All Cells
 ...
 *
 * The success marker is the notebook's own last cell: it asserts
 * `parallel_seconds < serial_seconds` and then prints
 * `POOL_DEMO_OK <speedup>` — so the marker's presence already proves the
 * parallel run beat the serial baseline inside the kernel.
 */
...
test('01-pool-basics.ipynb: Run All Cells prints POOL_DEMO_OK with no errors', async ({
  ...
  await page.goto('/lab/index.html?path=01-pool-basics.ipynb')
  ...
  // upstream (wheel install, nested-worker pool boot, serial + parallel runs,
  // matplotlib download) has to finish first, so this is the long wait.
  const marker = /POOL_DEMO_OK (\d+(?:\.\d+)?)/
  await expect
    .poll(
      async () => {
  ...
  // dependency chain (chunks → serial_seconds → speedup), so a skipped or
  // failed cell surfaces as a NameError downstream, never as a silent pass.
  const cells = await readNotebookModel(page)
  ...
  await expect(async () => {
    ...
    // Scoped to output areas: the cell's own source (`print("POOL_DEMO_OK"…`)
    // also contains the string, and `\d` keeps the editor's tokenized
    // string-literal span from matching.
    await expect(
      page.locator('.jp-OutputArea-output', { hasText: /POOL_DEMO_OK \d/ }),
