...
#: Never mirrored to workers: ``pyodide_pool`` and the Phase 06
#: ``wasm_multiprocessing`` shim travel by value inside task payloads (they
#: are never installed on workers, see each package's ``__init__``), the
#: worker's execPickled shim loads cloudpickle itself, and micropip is the
#: install mechanism, not a task dependency. The second group is JupyterLite kernel
...
EXCLUDED_FROM_MIRROR = frozenset(
    {"pyodide_pool", "wasm_multiprocessing", "cloudpickle", "micropip"}
